...
const KIND = {
  "cognate": {
    glyph: "🤝",
    label: "Cognate",
    pedagogy: "Transfer directly",
    cls: "k-cognate",
    blurb: "Same idea — your intuition carries over.",
  },
  "false-friend": {
    glyph: "🎭",
    label: "False friend",
    pedagogy: "Flag the trap",
    cls: "k-false",
    blurb: "Looks familiar; behaves differently.",
  },
  "antonym": {
    glyph: "🔄",
    label: "Antonym",
    pedagogy: "Remap the intuition",
    cls: "k-antonym",
    blurb: "Related but inverted — flip your expectation.",
  },
  "alien-realization": {
    glyph: "🛸",
    label: "Alien realization",
    pedagogy: "Bridge with effort",
    cls: "k-alien",
    blurb: "Same goal, unfamiliar machinery.",
  },
  "novel": {
    glyph: "✨",
    label: "Novel",
    pedagogy: "Teach de novo",
    cls: "k-novel",
    blurb: "No prior anchor — learn it fresh.",
  },
  "vanished": {
    glyph: "👻",
    label: "Vanished",
    pedagogy: "Un-learn / re-route",
    cls: "k-vanished",
    blurb: "A habit to drop — it's gone here.",
  },
}
const RESIDUE = {
...
function renderIndex() {
  ...
  if (items.length === 0) {
    list.append(
      node("li", { class: "empty", role: "presentation", text: "No correspondences match." }),
    )
    renderDetail(null)
    return
  }
  ...
  items.forEach((it, i) => {
    const li = node("li", {
      ...
      node("div", {
        class: "pair",
        text: `${it.from?.language ?? "?"} → ${it.to?.language ?? "?"}`,
      }),
      kindBadge(it.kind),
    ])
...
function renderForms(it) {
  ...
  if (!it) {
    host.append(node("div", { class: "empty", text: "Select a correspondence." }))
    return
  }
  ...
  if (state.view === "focus") {
    host.append(node("div", { class: "focus-form" }, [
      node("p", {
        class: "pane-hint",
        text: `from your ${it.from?.language}: ${it.from?.surface}`,
      }),
      formCard(it.to),
    ]))
    return
  ...
  if (state.view === "glyph") {
    host.append(node("div", { class: "glyph-view" }, [
      ...
      node("div", { class: "row" }, [
        node("div", {}, [
          node("div", { class: "form-lang", text: it.from?.language }),
          node("code", { text: it.from?.surface }),
        ]),
        node("div", { class: "cross", text: "→" }),
        node("div", {}, [
          node("div", { class: "form-lang", text: it.to?.language }),
          node("code", { text: it.to?.surface }),
        ]),
      ]),
    ]))
    return
  ...
  if (state.view === "blockly") {
    host.append(node("div", { class: "blockly" }, [
      node("div", { class: "block" }, [
        node("div", { class: "blang", text: it.from?.language }),
        node("code", { text: it.from?.surface }),
      ]),
      node("div", { class: "connector", text: `▼ ${k.glyph ?? ""} ${k.label ?? it.kind}` }),
      node("div", { class: "block" }, [
        node("div", { class: "blang", text: it.to?.language }),
        node("code", { text: it.to?.surface }),
      ]),
    ]))
    return
  ...
  host.append(node("div", { class: "sxs" }, [
    formCard(it.from),
    node("div", { class: "cross" }, [
      document.createTextNode(k.glyph ?? "→"),
      node("small", { text: "becomes" }),
    ]),
    formCard(it.to),
  ]))
}
...
function renderClassify(it) {
  ...
  host.replaceChildren()
  if (!it) {
    host.append(node("div", { class: "empty", text: "—" }))
    return
  }
  ...
  const strataSection = node("div", { class: "section" }, [
    node("h3", { text: "Strata (levels of objects)" }),
    strataRow,
  ])
  if (isFalseFriendShape(it.strata)) {
    strataSection.append(
      node("div", {
        class: "ff-flag",
        text: "⚠ False-friend signature: corresponds at the surface, diverges at intention.",
      }),
    )
  }
  ...
  if (it.narrative) {
    const blocks = NARR.filter(([key]) => it.narrative[key])
      .map(([key, label]) =>
        node("div", { class: "n" }, [
          node("b", { text: label }),
          document.createTextNode(it.narrative[key]),
        ])
      )
    if (blocks.length) {
      host.append(
        node("div", { class: "section" }, [
          node("h3", { text: "Narrative (no shame)" }),
          node("div", { class: "narrative" }, blocks),
        ]),
      )
    }
  }
  if (it.witness) {
    host.append(
      node("div", { class: "section" }, [
        node("h3", { text: "Witness" }),
        node("code", { text: it.witness }),
      ]),
    )
  }
}
...
async function init() {
  ...
  document.querySelectorAll("input[name=view]").forEach((r) =>
    r.addEventListener("change", (e) => {
      ...
      $("viewHint").textContent = e.target.value
